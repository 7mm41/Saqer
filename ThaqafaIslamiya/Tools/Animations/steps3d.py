...
def build_feet(st, m):
    ...
    L = st.leg
    L.capsule('shin', 0.5); L.ellipsoid('foot', 1.0, (1.1, 0.5, 0.36)); L.ball('heel', 0.55)
    for k in range(5):
...
def feet_pose(st, t, mirror):
    ...
    L.place('heel', V(1.25, -0.4, 0.95), radius=0.6)
    L.place('foot', V(0.1, -0.45, 0.92), radius=1.0, size=(1.45, 0.66, 0.42))
    for k in range(5):
        L.place('toe%d' % k, V(-0.95 + k * 0.06, -0.78 + k * 0.17, 0.9 - k * 0.02), radius=0.24 - k * 0.018)
    ...
    hand = V(-0.6, -0.9, 1.45).lerp(V(0.85, -0.85, 1.75), p)
    shoulder = V(3.6, -1.2, 6.0)
    elbow, wrist = R3.ik3(shoulder, hand, 2.6, 2.4, V(1.5, -1, 0))
    st.sleeve.span('arm', shoulder, elbow, 0.62); st.sleeve.place('cuff', elbow, radius=0.58)
    L.span('fa', elbow, wrist, 0.36)
    L.place('hand', wrist + V(-0.28, -0.05, -0.1), radius=1.0, size=(0.5, 0.45, 0.2))
    for k in range(4):
        a0 = wrist + V(-0.6, -0.25 + k * 0.17, -0.18)
        L.span('f%d' % k, a0, a0 + V(-0.4, 0, -0.08), 0.11)
    L.span('thumb', wrist + V(-0.3, -0.45, -0.05), wrist + V(-0.6, -0.55, -0.15), 0.12)
